...
//task 2
//Ожидает поднятия семафора от первой задачи, увеличивает переменную (N = N +
1)
//и отправляет в буфер последовательность из N байт (1,2,3,..N) каждые 50 мс.
//Затем управляет светодиодом, который мигает N раз (ON — 300 мс, OFF — 500
мс).
int task_N_cnt = 0;
...
task_N_cnt++;
//отправляем в очередь последовательность из N байт (1,2,3,..N)
каждые 50 мс
